...
int main(int argc, char* argv[]){
   ...
   //Initialize 
    ...
    if(rank == 0){
        ...
        //Receive message and end time 
        char *value_buffer = new char[length];
        MPI_Irecv(value_buffer, length, MPI_CHAR, 1, 0, MPI_COMM_WORLD, &iReq);
        MPI_Wait(&iReq, &iStatus);
        value = value_buffer;
        endTime = MPI_Wtime();
        ...
        std::string tempVal = value_buffer;

        //Send message
        MPI_Isend(tempVal.c_str(), length, MPI_CHAR, 0, 0, MPI_COMM_WORLD, &iReq);
        MPI_Wait(&iReq, &iStatus);
    }
